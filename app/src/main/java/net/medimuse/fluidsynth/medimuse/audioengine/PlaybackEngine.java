package net.medimuse.fluidsynth.medimuse.audioengine;
...
public class PlaybackEngine  {
    ...
    public static int loadSoundFont (String fontFile) {
        if (mEngineHandle != 0) {
            return native_loadSoundFont(mEngineHandle,fontFile);
        }
        return -1;
    }

    public static void onSend(byte[] msg, int offset, int count, long timestamp) {
        if (mEngineHandle != 0) {
            native_onSend(mEngineHandle,msg,offset,count,timestamp);
        }
    }

    /**
     * Call this to reset synth, usually when midi client disconnects or on startup
     */
    public static void resetSynth(){
        if (mEngineHandle != 0) {
            native_resetSynth(mEngineHandle);
        }
    ...
    private static native int native_loadSoundFont(long engineHandle,String fontFile);
    private static native void native_onSend(long engineHandle,byte[] msg,int offset, int count, long timestamp);
    private static native void native_resetSynth(long engneHandle);
