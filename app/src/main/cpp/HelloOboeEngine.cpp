...
void HelloOboeEngine::resetSynth() {
    if(mAudioSource != nullptr) {
        FluidSynthesizer::getInstance()->resetSynth();
    }
...
int HelloOboeEngine::loadSoundFont(const char *fontFile) {



    if(mAudioSource != nullptr) {
        return FluidSynthesizer::getInstance()->loadSoundFont(fontFile);
    } else {
        return FLUID_FAILED;
    }
}
...
oboe::Result HelloOboeEngine::createPlaybackStream() {
    ...
    return builder.setSharingMode(oboe::SharingMode::Shared)
...
void HelloOboeEngine::onSend(uint8_t *msg,int offset,int count, long timeStamp) {
    if(mAudioSource != nullptr) {
        FluidSynthesizer::getInstance()->onSend(msg,offset,count,timeStamp);
    }
